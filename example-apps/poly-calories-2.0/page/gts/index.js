...
Page({
  buildTopContent(calories) {
    ...
    // polyglot -> parse text for a "unit" keyword
    const { width: w2 } = getTextLayout(poly.getText('unit'), {
  ...
  build() {
    ...
    // show polybubble on application start
    // restart the app on new language selection
    poly.showPolyBubble({ restart_app: true });
  },
